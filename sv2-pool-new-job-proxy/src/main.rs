...
async fn main() {
    ...
    let server_address: String = env::var("SERVER").expect("SERVER environment variable not set");

    let gauge: Arc<Mutex<GenericGauge<AtomicF64>>> = Arc::new(Mutex::new(
        register_gauge!(
            "sv2_pool_new_job_latency_milliseconds",
            "New Job sv2 pool latency in milliseconds"
        )
        .unwrap(),
    ));
    ...
    tokio::spawn(async move {
        ...
        warp::serve(metrics_route).run(([0, 0, 0, 0], 3497)).await;
    });
...
async fn connect_to_server(server_address: &str) -> TcpStream {
    ...
    let res: TcpStream = TcpStream::connect(address).await.unwrap();
    res
}
