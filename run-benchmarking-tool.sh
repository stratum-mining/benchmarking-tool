...
if [[ "$CONFIG" == "A" ]]; then
    ...
    echo "How often do you want your local Job Declarator Client (JDC) to produce updated templates? (default is '30'):"
    DEFAULT_INTERVAL=$DEFAULT_INTERVAL_A
else
    echo "The SV1 pool used in the benchmarking tool will generate a new block template every 60 seconds."
    echo "How often do you want the SV2 pool to send updated block templates? This value will affect the bandwidth used. (default is '60'):"
    DEFAULT_INTERVAL=$DEFAULT_INTERVAL_C
fi

# Prompt user for block template update interval for SV2 with default value
read -p "Enter the interval in seconds for updating the block template for SV2 (default is '$DEFAULT_INTERVAL'): " SV2_INTERVAL
...
echo "Now point your miner to the SV1 pool: stratum+tcp://<host-ip-address>:3333"
echo "And point your miner to the SV2 pool: stratum+tcp://<host-ip-address>:34255"
echo ""
